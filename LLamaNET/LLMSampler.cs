...
public abstract class LLMSampler {
    ...
    public LLMToken Sample(LLamaContext context, ReadOnlySpan<LLMToken> LastToken) {
        LLamaCandidates candidates = new(context);

        // 로짓 프로세스 및 패널티 적용
        ApplyPenalty(ref candidates, LastToken);
    ...
    public abstract void ApplyPenalty(ref LLamaCandidates candidates, ReadOnlySpan<LLMToken> LastToken);

    /// <summary>토큰 후보를 통해 최종 토큰을 샘플링합니다.</summary>
    /// <param name="candidates">토큰 후보입니다.</param>
    /// <returns>선별된 토큰입니다.</returns>
    public abstract LLMToken Sample(LLamaCandidates candidates);
}